...
if pipeline is None:
    st.sidebar.warning(
        "No pipeline detected. Please run training to create models/final_pipeline.joblib."
    )
else:
...
# ----------------- LEFT: file upload & batch prediction -----------------
with col1:
    uploaded_files = st.file_uploader(
        "Upload CSV / Excel files (multi allowed)",
        type=["csv", "xlsx", "xls"],
    ...
    if uploaded_files:
        if st.button("Predict Uploaded Files"):
            if pipeline is None:
                st.error(
                    "No pipeline available. Train and save pipeline to models/final_pipeline.joblib."
                )
            else:
                for uploaded_file in uploaded_files:
                    ...
                    features_df = df.drop(
                        columns=params["data_processing"]["passthrough_features"],
                        errors="ignore",
                    )
                    try:
                        preds = pipeline.predict(features_df)
                        probs = (
                            pipeline.predict_proba(features_df)
                        ...
                        if probs is not None:
                            ...
                            out_df = pd.concat(
                                [out_df, probs_df.reset_index(drop=True)], axis=1
                            )
                            out_df["Confidence"] = probs.max(axis=1)
                        st.dataframe(out_df.head(20))
                        csv = out_df.to_csv(index=False).encode("utf-8")
                        st.download_button(
                            f"Download predictions for {uploaded_file.name}",
                            data=csv,
                        ...
                        if hasattr(pipeline, "feature_names_in_"):
                            ...
                            if missing:
                                st.info(
                                    f"Missing input fields expected by model: {sorted(list(missing))}"
                                )
                        else:
...
with col2:
    ...
    sample_input = st.text_area(
        "Paste sample here", height=200, placeholder="e.g.\nSiO2=45\nAl2O3=15\nTiO2=0.5\n..."
    )
                ...
                if manual_mode == "Key=Value lines":
                    ...
                    for L in lines:
                        if "=" in L:
                            ...
                            raise ValueError(
                                "Invalid line format. Expect key=value per line."
                            )
                    sample_df = pd.DataFrame([d])
                else:
                    txt = sample_input.strip()
                    if "," in txt:
                        # try reading as CSV snippet with optional header
                        try:
                            sample_df = pd.read_csv(io.StringIO(txt))
                        except Exception:
                            vals = [v.strip() for v in txt.split(",")]
                            # try to use expected columns from pipeline if available
                            expected = None
                            if hasattr(pipeline, "feature_names_in_"):
                                expected = list(pipeline.feature_names_in_)
                            if expected and len(vals) == len(expected):
                                sample_df = pd.DataFrame(
                                    [dict(zip(expected, map(float, vals)))]
                                )
                            else:
                                raise ValueError(
                                    "CSV line does not match expected columns. Provide header or key=value lines."
                                )
                    else:
            ...
            if sample_df is not None:
                try:
                    ...
                    if label_encoder is not None:
                        ...
                        pred_label = preds[0]
                        class_names = (
                            list(pipeline.classes_) if hasattr(pipeline, "classes_") else []
                        )
...
if expected_cols:
    ...
    st.write(
        "Could not infer expected raw input column names automatically. Ensure your input includes the same raw oxide/REE column names used during training."
    )
